 ...
 * @author Mike
 */
class OpsViewDnDController {
	
	private static final Transfer[] EMPTY_TRANSFER_ARRAY = new Transfer[0];
	
	OpsViewDnDController( final TreeViewer viewer ) {
		assert viewer != null;
		...
		target.addDropListener ( new DropTargetAdapter() {
			...
			public void dragOver(DropTargetEvent event){
				if ( event.item != null && event.item instanceof TreeItem ) {
					...
					if ( item.getData() instanceof OpsViewParameterProxy ) {
						Transfer transfer =
							((OpsViewParameterProxy)item.getData()).getTransfer();
					...
					if ( item.getData() instanceof OpsViewActionProxy ) {
						if ( !item.getExpanded() ) {
							// only expand if one or more params fits the 
							// dragged item
							boolean expand = false;
							paramLoop: for ( OpsViewParameterProxy param :
									((OpsViewActionProxy)item.getData()).getParams() ) {
			...
			public void drop(DropTargetEvent event) {
				TreeItem item = (TreeItem)event.item;
				if ( event.data != null &&
						item.getData() instanceof OpsViewParameterProxy ) {
					OpsViewParameterProxy param =
						((OpsViewParameterProxy)item.getData());
	...
	// commonly used
	private OpsViewParameterProxy getSelectedParam( TreeViewer viewer) {
		assert viewer != null;
    	if ( viewer.getSelection().isEmpty() ) return null;
    	Object obj = ((StructuredSelection)
    			viewer.getSelection()).getFirstElement();
    	if ( obj instanceof OpsViewParameterProxy ) {
    		return (OpsViewParameterProxy)obj;
    	}
    	return null;
	}
