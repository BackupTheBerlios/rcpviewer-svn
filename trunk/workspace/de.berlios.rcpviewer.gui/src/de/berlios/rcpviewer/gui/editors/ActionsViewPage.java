...
import de.berlios.rcpviewer.gui.jobs.AbstractUserJob;
	...
	public void createControl(Composite parent) {
		...
		_viewer = new TreeViewer( parent, style){
			...
			protected void hookControl( final Control control) {
				final ActionsViewCellEditorFactory factory
					= new ActionsViewCellEditorFactory();
		...
		_viewer.setColumnProperties( new String[] { 
				ActionsViewCellModifier.LABEL_COLUMN, 
				ActionsViewCellModifier.VALUE_COLUMN } );
		_viewer.setCellModifier( new ActionsViewCellModifier(_viewer  ) );
		...
		Job resize = new AbstractUserJob( "TreeColumnResize") { //$NON-NLS-1$
			public IStatus runInUIThread(IProgressMonitor monitor) {
				SWTUtils.autosizeLastColumn( _viewer.getTree() );
